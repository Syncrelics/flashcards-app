...
if uploaded_file and st.sidebar.button("Process Document"):
    with st.spinner("Extracting text and generating embeddings..."):
        ...
        if not all_text.strip():
            st.sidebar.error("⚠️ No text could be extracted from this PDF. Please check the file.")
            st.stop()
...
with tab_gen:
    ...
    with col1:
        topic = st.text_input("What specific concept do you want to study?", placeholder="e.g., Lucknow Pact or Home Rule League")
    with col2:
        num_cards = st.slider("Number of cards", min_value=1, max_value=10, value=3)

    if topic and st.button("Generate Flashcards"):
        if collection.count() == 0:
            st.warning("Your database is empty. Please upload and process a PDF in the sidebar first.")
        else:
        ...
        if st.button("Submit Answer"):
            if not user_answer.strip():
                ...
                with st.spinner("Grading your answer..."):
                    ...
                    grade_prompt = f"""You are an expert tutor grading a student's flashcard response.
                    
Question: {current_card['q']}
Official Correct Answer: {current_card['a']}
Student's Answer: {user_answer}

Evaluate the student's answer. Give a score out of 10 and a brief, encouraging feedback paragraph explaining what they got right and what they might have missed or misunderstood."""
